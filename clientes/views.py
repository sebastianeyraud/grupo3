...
def modificar_perfil_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            mensaje = "Perfil actualizado correctamente. Por favor, vuelve a iniciar sesión."
            request.session.flush()
            return redirect('inicio')
        except Exception as e:
            mensaje = f"Error al modificar perfil: {e}"

    return render(request, 'modificar_perfil.html', {'mensaje': mensaje})
def modificar_perfil_view(request):
